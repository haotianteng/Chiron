...
def save_model(log_dir, model_name):
    ...
    TODO: Save the model hyperparameters in Json file.
    Args:
...
def inference(x, seq_len, training, rnn_layer_num=3):
    ...
    if rnn_layer_num == 0:
        ...
        logits = rnn_layers(cnn_feature, sequence_len,
                            training, layer_num=rnn_layer_num)
    return logits, ratio


def dense2sparse(label):
    """Transfer the dense label tensor to sparse tensor.
    Args:
        label: Tensor of shape [batch_size, MAX_LABEL_LEN], padded with -1.
...
def loss(logits, seq_len, label):
    ...
    loss = tf.reduce_mean(tf.nn.ctc_loss(
        label, logits, seq_len, ctc_merge_repeated=True, time_major=False))
...
def prediction(logits, seq_length, label, top_paths=1):
    ...
    predict = tf.nn.ctc_beam_search_decoder(
        logits, seq_length, merge_repeated=False, top_paths=top_paths)[0]
    edit_d = list()
    for i in range(top_paths):
        tmp_d = tf.edit_distance(tf.to_int32(
            predict[i]), label, normalize=True)
        edit_d.append(tmp_d)
...
def train(hparam):
    ...
    x, seq_length, train_labels = inputs(
        hparam.data_dir, hparam.batch_size, for_valid=False)
    ...
    for i in range(hparam.max_steps):
        ...
        if i % 10 == 0:
            ...
            saver.save(sess, hparam.log_dir + hparam.model_name +
                       '/model.ckpt', global_step=global_step_val)
