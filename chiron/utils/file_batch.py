...
def extract():
    ...
    def extract_fast5(input_file_path,bin_h,mode = 'DNA'):
        ...
        """
        try:
            (raw_data, raw_label, raw_start, raw_length) = labelop.get_label_raw(input_file_path,FLAGS.basecall_group,FLAGS.basecall_subgroup)
        except IOError:
        ...
        for index,start in enumerate(raw_start):    
            if start-pre_start > FLAGS.length:
                if index-1 == pre_index:
                    ...
                    pre_index = index
                    continue
                event.append(np.pad(raw_data[pre_start:raw_start[index-1]],(0,FLAGS.length+pre_start-raw_start[index-1]),mode = 'constant'))
                ...
                label.append(np.pad(temp_label,(0,FLAGS.length - index+1+pre_index),mode = 'constant',constant_values = -1))
        ...
        while len(event) > FLAGS.batch:
            for index in range(0,FLAGS.batch):
                bin_h.write(struct.pack(format_string,*[event_length[index]]+event[index].tolist()+[label_length[index]]+label[index].tolist()))
...
if __name__=="__main__":
    ...
    parser.add_argument('-i','--input',default = "/media/haotianteng/Linux_ex/Nanopore_Data/Lambda_R9.4/pass/", help="Directory that store the fast5 files.")
    parser.add_argument('-o','--output',default = "/media/haotianteng/Linux_ex/Nanopore_Data/Lambda_R9.4/file_batch_test/",help = "Output folder")
    parser.add_argument('--basecall_group',default = 'RawGenomeCorrected_000',help = 'Basecall group Nanoraw resquiggle into. Default is Basecall_1D_000')
    ...
    parser.add_argument('-n','--normalization',default = 'median',help="The method of normalization applied to signal, Median(default):robust median normalization, 'mean': mean normalization, 'None': no normalizaion")
